...
#define REPORT_OPTION  1
#ifdef REPORT_OPTION
  static bool ReportOption = true;
#ifdef USE_RK  
  static const char* RunType= "Runge-Kutta field propagation";
#else  
  static const char* RunType= "Helix field propagation";
#endif  
  if( blockIdx.x == 0 && threadIdx.x == 0 && ReportOption ) {
     printf( "-- Run type: %s .\n\n", RunType );
     ReportOption= false;
  }
#endif
...
#else    
    Precision BzFieldValue= *gPtrBzFieldValue_dev;
    if (BzFieldValue != 0.0) {
