...
int main(int argc, char **argv)
{
  ...
  vecgeom::Vector3D<float> magFieldVec(BfieldValue[0], BfieldValue[1], BfieldValue[2]);
  ...
  for (int i = 0; i < SmallNum; i++) {
    ...
    if (badPosition || badDirection) {
      std::cout << std::endl;
      std::cout << " Difference seen for Track " << i << " addr = " << &(*trackBlock_uniq)[i] << std::endl;
      std::cout << std::endl;
      ...
      if (badPosition) {
        std::cout << " Position  diff = " << posDiff << " mag = " << posDiff.Mag() << " vs move      = " << move << " "
                  << std::endl;
      }
      if (badDirection) {
        std::cout << " Direction diff = " << dirDiff << " mag = " << dirDiff.Mag() << " vs deflection = " << deflection
                  << " " << std::endl;
      }
      std::cout << std::endl;
  ...
  std::cout << " maxIndex = " << maxIndex << " numTracks = " << numTracks << std::endl;
